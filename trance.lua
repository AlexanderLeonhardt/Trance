...
local function load_palette(palette_name)
    local base_path = lovely.mod_dir .. "/Trance/colors/"
    local pack_code = nativefs.read(base_path .. palette_name .. ".lua")
    if not pack_code then
        -- fallback to Base Game.lua
        pack_code = nativefs.read(base_path .. "Base Game.lua")
        if not pack_code then
            error("Could not load any palette file including Base Game.lua")
        end
        palette_name = "Base Game"
        Trance_config.palette = "Base Game" 
    end
    local chunk = load(pack_code)
    if not chunk then
        error("Failed to load palette chunk for " .. palette_name)
    end
    return chunk()
end
...
Trance = load_palette("Base Game")
Trance_theme = load_palette(Trance_config.palette)
